...
public class AddonScanner implements AutoCloseable {
    ...
    private final Set<String> workspaceBaseline;

    public AddonScanner() {
        this.workspaceRoot = Paths.get("").toAbsolutePath().normalize();
        this.workspaceBaseline = listTopLevelNames(this.workspaceRoot);
    }
    ...
    public JarScanResult scan(Path jarPath) {
        ...
        resetRuntime();
        RuntimeSandbox sandbox = RuntimeSandbox.open(absoluteJar, workspaceRoot);
        if (sandbox.openError() != null) {
            result.warnings.add("Runtime sandbox setup failed; using current directory. " + sandbox.openError());
        }
        ...
        if (sandboxEnterError != null) {
            result.warnings.add("Runtime sandbox activation failed; using current directory. " + sandboxEnterError);
        }
        ...
        try (ChildFirstClassLoader loader = new ChildFirstClassLoader(new URL[]{jarUrl}, AddonScanner.class.getClassLoader())) {
            ...
            for (String rawEntrypoint : metadata.meteorEntrypoints) {
                ...
                try {
                    ...
                    addon.name = metadata.name != null ? metadata.name : (metadata.id != null ? metadata.id : entrypoint);
            ...
            for (EntrypointHolder holder : loadedEntrypoints) {
                try {
                    ...
                    result.errors.add("onInitialize failure (" + holder.rawEntrypoint + "): " + rootMessage(t));
                }
            }
            ...
            modules.sort(Comparator.comparing((Module m) -> safe(m.category == null ? null : m.category.name)).thenComparing(m -> safe(m.name)));
    ...
    private static String rootMessage(Throwable t) {
        ...
        for (StackTraceElement frame : trace) {
            ...
            if (owner.startsWith("java.") || owner.startsWith("javax.") || owner.startsWith("jdk.") || owner.startsWith("sun.")) continue;
            interesting = frame;
            break;
        }
        return base + " @" + interesting.getClassName() + "#" + interesting.getMethodName() + ":" + interesting.getLineNumber();
    }
    ...
    private static Set<String> listTopLevelNames(Path root) {
        HashSet<String> names = new HashSet<>();
        if (root == null || !Files.isDirectory(root)) return names;
    ...
    private static boolean cleanupNewTopLevelArtifacts(Path workspaceRoot, Set<String> baseline, Path protectedRoot) {
        boolean deletedAny = false;
        Set<String> after = listTopLevelNames(workspaceRoot);
        Path protectedNormalized = protectedRoot == null ? null : protectedRoot.toAbsolutePath().normalize();

        for (String name : after) {
            if (baseline.contains(name)) continue;

            Path candidate = workspaceRoot.resolve(name).toAbsolutePath().normalize();
            if (protectedNormalized != null && (candidate.startsWith(protectedNormalized) || protectedNormalized.startsWith(candidate))) {
                continue;
            }
    ...
    public void close() {
        ...
        for (int pass = 0; pass < 40; pass++) {
            boolean deleted = cleanupNewTopLevelArtifacts(workspaceRoot, workspaceBaseline, null);
            if (!deleted && pass >= 5) break;
            try {
                Thread.sleep(250L);
    ...
    private static final class RuntimeSandbox {
        ...
        private final Path workspaceRoot;
        private final Set<String> workspaceEntriesBefore;
        ...
        private RuntimeSandbox(Path rootDir, Path scanDir, Path workspaceRoot, Set<String> workspaceEntriesBefore, boolean keepArtifacts, String openError) {
            this.rootDir = rootDir;
            this.scanDir = scanDir;
            this.workspaceRoot = workspaceRoot;
            this.workspaceEntriesBefore = workspaceEntriesBefore;
            this.keepArtifacts = keepArtifacts;
            this.openError = openError;
        }

        static RuntimeSandbox open(Path jarPath, Path workspaceRoot) {
            Path root = resolveTmpRoot();
            boolean keep = shouldKeepTmp();
            Set<String> before = listTopLevelNames(workspaceRoot);
            try {
                ...
                return new RuntimeSandbox(root, scan, workspaceRoot, before, keep, null);
            } catch (Throwable t) {
                return new RuntimeSandbox(root, null, workspaceRoot, before, keep, rootMessage(t));
            }
        }
        ...
        String enter() {
            ...
            try {
                ...
                if (previousMeteorFolder != null) meteordevelopment.meteorclient.MeteorClient.setFolder(previousMeteorFolder);
        ...
        void exit() {
            if (entered) {
                if (previousUserDir != null) System.setProperty("user.dir", previousUserDir);
                meteordevelopment.meteorclient.MeteorClient.setFolder(previousMeteorFolder == null ? new File(".") : previousMeteorFolder);
                entered = false;
            }
            ...
            if (!keepArtifacts) {
                ...
                cleanupNewTopLevelArtifacts(workspaceRoot, workspaceEntriesBefore, rootDir);
            }
        }
    }
}
